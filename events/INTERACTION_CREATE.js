  ...
  try {
    ...
    if (!content) {
      ...
      async function getMember(userid) {
        if (!userid) return;
        let member = client.guilds.cache.get(interaction.guild_id).members.cache.get(userid)
        return member;
      }
      ...
      if (args) {
        ...
        interaction.getUser = async (userid) => await getUser(userid)

        interaction.getMember = async (userid) => await getMember(userid)
