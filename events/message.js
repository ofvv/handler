...
let normalPrefix = config.bot.prefixes.normalPrefix;
let normalPrefix2 = config.bot.prefixes.normalPrefix2;
let owner = config.bot.owners.owner;
let owner2 = config.bot.owners.owner2;
let owner3 = config.bot.owners.owner3;
let errchannel = config.channels.errchannel;
let nitroch = config.channels.nitroch;
...
module.exports.run = async (client, message) => {
  try {
    ...
    let getstartedprefix;
    if (!message.guild) getstartedprefix = normalPrefix;
    else {
      getstartedprefix = await db.get(`guild_prefix_${message.guild.id}`);
    }
    if (getstartedprefix === null | getstartedprefix === undefined) getstartedprefix = normalPrefix;
    if (!message.author.bot && message.content.match(`^<@!?${client.user.id}>( |)$`)) {
      const prefixembed = new MessageEmbed()
        .setDescription(`**My Prefix Is \`${getstartedprefix}\`, You Can Also Use My Mention Prefix! Use ${getstartedprefix}help To Get Started!**`)
        .setColor('#000001');
      if (message.guild) {
    ...
    if (!command.cooldown.dbtype) command.cooldown.dbtype = 'local';

    if (!command.cooldown) {
      command.cooldown = {
        db: command.cooldown.db || false,
        dbtype: command.cooldown.dbtype || "local",
    ...
    //-------------BLACKLIST SYSTEM-------------
    if (command.user && command.user.age) {
      let time = Date.now() - message.author.createdTimestamp;
      if (time < command.user.age) {
        return message.lineReplyNoMention(`**Your Account Is Too New To Use This Command!**`)
      }
    };
    ...
    //-------------COOLDOWN SYSTEM-------------
    if (!command.cooldown.dbtype || !command.cooldown || !command.cooldown.cd || !command.cooldown.db) {
      command.run(client, message, args, prefix);
    } else if (command.cooldown && command.cooldown.db === true && command.cooldown.dbtype === 'mongo') {
      ...
      if (cooldown !== null && timeout - (Date.now() - cooldown) > 0) {
        ...
        setTimeout(async function() {
          await db.delete(`cooldown_${command.name}_${message.author.id}`);
        }, timeout - (Date.now() - cooldown) || 1)
    ...
    } else if (command.cooldown && command.cooldown.db === true && command.cooldown.dbtype === 'local' || command.cooldown && command.cooldown.db === false && !command.cooldown.dbtype) {
      ...
      if (command) command.run(client, message, args, prefix);
    } else if (command.cooldown && command.cooldown.db === true && command.cooldown.dbtype === 'json') {
      const timeout = command.cooldown.cd || 0;
      const cooldown = await client.dbs.json.get(`cooldown_${command.name}_${message.author.id}`);
      ...
      if (cooldown !== null && timeout - (Date.now() - cooldown) > 0) {
        ...
        setTimeout(async function() {
          await client.dbs.json.delete(`cooldown_${command.name}_${message.author.id}`);
        }, timeout - (Date.now() - cooldown) || 1)
        return message.lineReplyNoMention(`**You Are On Cooldown! Your Cooldown Will Expire in ${humanized}!**`).then(msg => msg.delete({
          timeout: 5000
        }));
        ...
        if (command) command.run(client, message, args, prefix);
        await client.dbs.json.set(`cooldown_${command.name}_${message.author.id}`, Date.now());
      }
    }
    //-------------GHOSTBOT SYSTEM-------------
    ...
    client.db.add(`${message.author.id}.cmds`, 1)
  } catch (e) {
    console.log(e);
  }
}
